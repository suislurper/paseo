...
Codex collaboration modes are selected by protocol `mode` identity (`code`, `default`, or `auto` for ordinary turns; `plan` for Plan). Display names are used only when an entry has no identity, and a name never overrides an explicit `plan` identity. Ordinary turns never fall back to the first catalog entry. If Codex advertises a catalog with no ordinary mode, `startTurn` fails before `turn/start` instead of silently running Plan. Clients that do not advertise collaboration modes keep running without a `collaborationMode` field.
