...
When Claude profiles use separate `CLAUDE_CONFIG_DIR` values, Paseo copies the active session transcript and its session artifacts into the target profile before resuming. Codex profiles work the same way with separate `CODEX_HOME` values: Paseo copies the session rollout files (including referenced sub-agent threads) into the target profile home before resuming. Paseo waits for the old Codex process to exit before taking the history snapshot; if the destination cannot start, it attempts to reopen the original account. The transfer keeps an existing target that already contains the source history. When the copies differ, it preserves the old target under `CODEX_HOME/paseo-profile-transfer-backups/<thread-id>/` and atomically publishes the selected source conversation. A source or target that changes during staging causes the switch to stop without publishing that snapshot. Only structured child-session references are followed; UUIDs quoted in ordinary conversation text do not trigger unrelated history copies. Credentials and other profile settings remain isolated; only the selected conversation is transferred.
