...
Create the agent via Paseo with a `[Handoff] <task>` title, the briefing as initial prompt, and `relationship: { kind: "detached" }`. Set an explicit permission mode; never inherit a provider default.
