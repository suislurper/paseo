...
- **This operator runs the canonical fork at `suislurper/paseo:main`.** Its multi-profile Claude support, modeless cross-provider behavior, per-profile usage, active-provider selection, landed-to-default workspace status, safe archive cleanup, and live-chat synchronization are required product behavior—not optional local patches. Never build or install the desktop app from `getpaseo/paseo` or from a branch that has not passed `npm run verify:operator-fork`. `npm run build:desktop` runs this check automatically and requires local `main` at the exact fetched `suislurper/paseo:main` commit; `--head` is ancestry-only review/testing mode and does not authorize packaging. Before replacing fork `main`, inventory every local and fork-side branch, integrate every operator feature, and update the executable feature manifest in `scripts/verify-operator-fork-baseline.mjs`; a previously green manifest is not proof that a newer side branch was included. Then run focused tests and an exact-SHA review. Never replace fork `main` with upstream history. Remove integrated temporary feature refs after landing so they cannot be mistaken for the canonical build. See [docs/operator-fork.md](docs/operator-fork.md).
- **The desktop AppImage has two independently built halves, and packaging the wrong way ships a stale UI without any error.** `resources/app.asar` comes from `packages/server/dist` (daemon, providers, model manifests); `resources/app-dist` is an Expo web export copied verbatim from `packages/app/dist` and contains the _entire_ UI. `npm run build --workspace=@getpaseo/desktop` rebuilds only the server half. **`npm run build:desktop` from the repo root is the only supported packaging path** — it is what runs `expo export`. If the export fails (commonly `Failed to resolve plugin for module "expo-gradle-jvmargs"`, meaning `node_modules` drifted from the lockfile), run `npm install` and re-run the export; never fall back to the workspace build. `verify:desktop-bundle` (prebuild) and `verify:desktop-packaged` (postbuild) now refuse a stale, empty, or marker-missing UI export, but they are a backstop, not the plan. See [docs/operator-fork.md](docs/operator-fork.md) for the runbook and known failure modes.
- **NEVER restart the main Paseo daemon on port 6767 without permission** — it manages all running agents. If you're an agent, restarting it kills your own process. This includes installing a new AppImage: install the artifact, then hand the restart to the operator.
