...
└── locks/
    └── {agentId}.lock/     # atomic mkdir lock directory (mode 0700); outside scratch/artifacts
        └── owner.json      # schemaVersion, agentId, lockToken, operation, pid, optional bootId, acquiredAt (mode 0600; no secrets)
```

`AgentRuntimeStorage` creates agent and locks-parent directories at mode `0700` (POSIX chmod is fail-closed), reuses a valid _active_ scratch generation across daemon restarts, and on `prepare` of a _released_ generation atomically rotates to a fresh active generation while retaining all scratch and artifact bytes. It fails closed on symlink roots/components, path escape, invalid agent IDs, malformed/mismatched manifests, non-directory collisions, or non-absolute runtime roots (native `path.isAbsolute` only). When `agents.runtimeRoot` is set, `AgentManager` constructs one storage instance and injects launch env (`TMPDIR`/`TMP`/`TEMP`, `PASEO_AGENT_SCRATCH_DIR`, `PASEO_AGENT_SCRATCH_GENERATION`, `PASEO_AGENT_ARTIFACT_DIR`, `PASEO_AGENT_ID`) from the common launch context after caller env so caller overrides lose; prepare failure aborts launch before the provider starts. When unset, no runtime directories are created and env injection is limited to `PASEO_AGENT_ID`.

**Per-agent filesystem lock:** `prepare`, `markReleased`, and the narrow `withAgentLock({ agentId, operation }, callback)` surface share one cross-process lock at `locks/{agentId}.lock`. Acquisition is atomic `mkdir` of that directory; a present, malformed, or symlink lock fails closed and is **never** broken by age. Owner metadata is a private `owner.json` (random lock token, operation, PID, optional Linux boot id, `acquiredAt` — no secrets). Release re-reads the exact token, removes only `owner.json`, then non-recursively removes the lock directory; token mismatch or unexpected contents fails closed and leaves the lock. The lock is held only during prepare/release **metadata** work — not while launching or running the provider. Future cleanup will hold the same lock across final revalidation and exact scratch removal. A crash may leave a stale lock directory; that intentionally blocks further prepare/release/cleanup for that agent until explicit operator inspection (remove the lock directory only after confirming no live holder).

**Scratch release (receipt only):** `AgentManager.releaseAgentScratch({ agentId, generation })` and the additive MCP tool `release_agent_scratch` mark an exact prepared generation `released` and return `{ agentId, generation, lifecycle: "released", releasedAt }`. Exact repeated release is idempotent (same `releasedAt`). Wrong generation, malformed ID, or unconfigured storage fail without mutation. Agent-scoped MCP sessions may release only their own `agentId`; global/operator sessions pass an explicit `agentId` and do not invent an owner. Release never archives the agent, closes a runtime, or deletes files; archive/tab close alone does not call release. Secrets must not be stored in either manifest.
